-- -----------------------------------------------------------------------------
-- 20260804000000_branch_menu_items_optional.sql
-- -----------------------------------------------------------------------------
-- A `branch_menu_items` row is NOT guaranteed to exist for a menu item. Only
-- dev seeding and onboarding create them, so every dish an owner adds through
-- the admin console has none. `menu.service.getAssembledMenu` already treats a
-- missing row as "available at base price" — but these two RPCs inner-joined
-- `branch_menu_items`, so they disagreed with the menu the diner was looking at:
--
-- 1. place_order() DROPPED such an item from the order. Silently: the dish
--    vanished from the ticket, the kitchen never saw it, and the subtotal was
--    recomputed from the surviving lines, so the table was under-charged with no
--    error anywhere. Reproduced live: an owner-created item added to the cart
--    was simply absent from the placed round.
--
-- 2. get_upsell_suggestions() could never suggest such an item — the entire
--    `available_items` CTE excluded it.
--
-- Fix: LEFT JOIN in both, and treat a missing row as available at base price,
-- matching getAssembledMenu. An explicit `available = false` (the 86 toggle)
-- still excludes the item, and `price_override` still wins where a row exists.
--
-- Both are CREATE OR REPLACE, so this is safe/idempotent on the live DB.
-- -----------------------------------------------------------------------------

--------------------------------------------------------------------------------
-- 1. place_order — never drop an item that has no branch override row
--------------------------------------------------------------------------------
...
BEGIN
  ...
  --    LEFT JOIN on branch_menu_items: no row means "no branch override", not
  --    "not on the menu". Dropping the line here loses the dish AND the money.
  FOR v_cart_item IN
    SELECT c.*,
           m.name AS item_name,
...
--------------------------------------------------------------------------------
-- 2. get_upsell_suggestions — owner-created items are suggestable too
--------------------------------------------------------------------------------
